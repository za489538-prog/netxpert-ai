...
import { isIPv6, isValidIPv6 } from './logic/ipv6-logic.js';
import { isValidIp } from './logic/subnet-logic.js';
...
export function initGamification() {
  ...
  document.querySelectorAll('[data-feature]').forEach(button => button.addEventListener('click', () => recordActivity(button.dataset.feature)));

  // calcBtn حالة خاصة: نفس الزر بيحسب IPv4 أو IPv6 حسب محتوى الإدخال، فلازم نميّز بينهم
  // وما نعطي نقاط إلا لو الإدخال صحيح فعلاً (مش فاضي أو غلط)
  document.getElementById('calcBtn')?.addEventListener('click', () => {
    const ipValue = document.getElementById('ipInput')?.value.trim() || '';
    if (!ipValue) return;
    if (isIPv6(ipValue)) {
      if (isValidIPv6(ipValue)) recordActivity('ipv6');
    } else if (isValidIp(ipValue)) {
      recordActivity('subnet');
    }
  });

  [['vlsmCalcBtn', 'vlsm'], ['compareBtn', 'compare'], ['findPathBtn', 'dijkstra']].forEach(([id, feature]) => document.getElementById(id)?.addEventListener('click', () => recordActivity(feature)));

  const badgeEl = document.getElementById('badgesList');
  if (badgeEl) badgeEl.innerHTML = BADGES.map(b => `<span class="badge" data-threshold="${b.threshold}">${b.name} · ${b.threshold} نقطة</span>`).join('');
}
